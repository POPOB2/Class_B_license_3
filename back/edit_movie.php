...
<form action="./api/add_movie.php" method="post" enctype="multipart/form-data"> <!-- enctype="multipart/form-data"> == 表單資料的多媒體類型 -->
<table style="width:75%; margin:auto">
    <tr>
        ...
        <td>
            ...
            <div><label>分級:</label>
                <select name="level">
                    <option value="普遍級" <?=($row['level']=='')?'selected':'';?>>普遍級</option>
                    <option value="輔導級" <?=($row['level']=='')?'selected':'';?>>輔導級</option>
                    <option value="保護級" <?=($row['level']=='')?'selected':'';?>>保護級</option>
                    <option value="限制級" <?=($row['level']=='')?'selected':'';?>>限制級</option>
                </select>
        </div>
            ...
            <select name="month">
                <!-- option[value='$']*12>{$} 如下 -->
                <option value="1" <?=$month==1?'selected':'';?>>1</option> <!-- 較為冗長的方法 -->
            ...
            <select name="day">
                <?="<option value='{$day}'>{$day}</option>";?> <!-- 較為簡短的方法 -->
                <!-- option[value="$"]*31>{$} 如下 -->
